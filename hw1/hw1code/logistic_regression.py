...
def compute_avglogL(X,y,beta):
    ...
    avglogL = np.sum(y * (X @ beta) - np.log(1 + np.exp(X @ beta))) / n
...
def getBeta_BatchGradient(train_x, train_y, lr, num_iter, verbose):
    ...
    for iter in range(0, num_iter):
        ...
        beta += lr * (train_x.T @ (train_y - sigmoid(train_x @ beta)))
        #========================#
...
def getBeta_Newton(train_x, train_y, num_iter, verbose):
    ...
    for iter in range(0, num_iter):
        ...
        first_deri = train_x.T @ (train_y - sigmoid(train_x @ beta))
        p_beta = sigmoid(train_x @ beta)
        second_deri = train_x.T @ np.diag(p_beta * (1 - p_beta)) @ train_x
        beta += np.linalg.inv(second_deri) @ first_deri
