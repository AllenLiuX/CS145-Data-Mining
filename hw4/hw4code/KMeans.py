    ...
    def getMaxClusterLabel(cluster):
        ...
        for point in cluster:
            if not point.label in labelCounts:
                ...
                labelCounts[point.label] += 1
        max = np.max(list(labelCounts.values()))
        return max

    for cluster in clusters:
        maxLabelCluster.append(getMaxClusterLabel(cluster))
    # ========================#
    #   END YOUR CODE HERE   #
    # ========================#
    purity = 0.0
...
class KMeans:
    ...
    def getCentroid(self, cluster):
        ...
        # STRART YOUR CODE HERE  #
        # ========================#
        cx = np.sum([d.x for d in cluster])/float(len(cluster))
        cy = np.sum([d.y for d in cluster])/float(len(cluster))
